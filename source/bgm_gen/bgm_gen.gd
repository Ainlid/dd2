...
func _set_sample():
	var m_id = globals.dream_rng.randi()%globals.samples.size()
	melody.stream = globals.samples[m_id]
	var b_id = globals.dream_rng.randi()%globals.samples.size()
	bass.stream = globals.samples[b_id]
